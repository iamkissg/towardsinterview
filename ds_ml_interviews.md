...
- According to Miklov
    - Skip-gram: 用目标单词去预测上下文单词, 在一个窗口内构造了很多单词对 (不算负样本的情况下, 预测 2n 次), 在小数据集上表现更好, 同时对于低频词的表示更好.
    - CBOW: 用上下文单词的平均向量表示来预测目标单词, 在一个窗口内相当于只刻画了一对单词关系 (不算负样本的情况下, 预测 1 次), 训练速度更快, 对于高频词的表示稍微更好一些(平均词向量法有一定的正则化效果?)

GRU vs. LSTM
===

- 同:
    - 两者都使用了**门控**的方式来缓解梯度消失问题.
- 异:
    - GRU 只使用了两个门, 更新门和重置门; LSTM 使用了三个门, 输入门, 遗忘门和输出门;
    - LSTM 有两种隐藏层状态 cell state 和 hidden state; GRU 只有一个 hidden state; (cell state 又被称作 internal memory)
    - 鉴于以上两方面原因, GRU 相较 LSTM 结构更简单, 参数更少, 训练速度更快;
    - 很多学者在不同任务上对比过 GRU 和 LSTM, 两者表现差不多, 并不存在一个明显优于另一个 (建议是, 调参可能比选择一个理想的架构带来的收益更大);
    - GRU 在数据集较小的情况下, 可能表现得更好, 因为它不容易过拟合;
    - 理论上, LSTM 能捕获更长距离的依赖
...
- 最显著的差别源于输入的不同:
    - NLP 是对一维信号 (句子, 单词序列) 进行操作;
    - CV 是对二维图像或者三维视频进行操作;
    - 因此, 发挥 CNN 局部连接的特点, NLP 用一维卷积, 而 CV 用二维或者三维卷积

近几年带来重大进步的技术
===

1. ResNet [ImageNet 2015 1st place](https://arxiv.org/abs/1512.03385)
2. Attention [ICLR 2015](https://arxiv.org/abs/1409.0473)
3. Dropout [JMLR 2014](https://dl.acm.org/citation.cfm?id=2670313)

Transformer vs. RNN
===

- RNN 之殇
    - RNN 训练很慢, 即使像 Cuda 在底层做了计算优化来加速计算, 碍于 RNN 随时间迭代的特点 (慢是天生的), 后一个时刻总是需要等待前一个时刻的计算完成, GPU 的利用率很低, 训练效率上很差.
        - 每个 LSTM 单元, 在每个时间步, 实际有 4 个线性层.
    - 尽管 LSTM 和 GRU 对普通 RNN 做了很多改进, 比如使用了门控信号来缓解梯度消失问题, 还是逃不脱用有限来保存无限的困境, 就像计算机表示浮点数, 字长多长都存在精度问题, 即使有 internal memory, 它还是会在随时间向前的过程中被更新, 维护不了很长的长期依赖
- 基于 Attention 的 Transformer 之利
    - 无论多长的序列, 开头和结尾都能发生交互, 不存在 RNN 长期依赖问题
    - 如有必要, 通过 mask 来控制后续信息的泄漏, 除此之外, attention 的计算可以利用矩阵乘法来加速计算, 计算效率高
    - 某种程度上突破了时间的限制, 基于 transfomrer 的模型可以从任意时刻向两边传递信息, 举个例子, 在自然语言生成中, 可以从任意时刻开始生成 (RNN 做不到, 因为 hidden state 是从头更新过来的)
    - 突破时间限制的另一个好处是, 双向 transformer (比如 BERT) 的双向意义和双向 RNN 的意义不一样, 前者是当前时刻之前之后的信息都可以补充到当前时刻, 而后者只能维护两条信息传递序列, 一条沿序列正向传播, 一条沿序列反向传播.
